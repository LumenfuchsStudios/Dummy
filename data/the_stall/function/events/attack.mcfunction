...
execute as @e[type=!#dummy_lib:technical, distance=..32] \
...
execute unless score @s dummy_lib.health matches 8 if predicate dummy_lib:random/1 \
		run scoreboard players add @s dummy_lib.health 1
...
# Play audio feedback
execute if predicate dummy_lib:random/5 run playsound ambient.soul_sand_valley.mood neutral @a[distance=..16] ~ ~ ~ 0.8 0.5
