...
# Remove self
execute if score @s lumenfuchs.settings matches 600.. run return run function dummy_lib:events/remove
...
# Wave limbs
execute if entity @s[tag=dummy_lib.dummy.is_walking] as @e[type=item_display, tag=dummy_lib.entity.dummy_limb, tag=!dummy_lib.dummy_limb.torso, tag=!dummy_lib.dummy_limb.head, distance=..3] if function dummy_lib:utils/is_matching_guid run function lumenfuchs:seeker/utils/wave_limb
execute if entity @s[tag=dummy_lib.dummy.is_hurt] as @e[type=item_display, tag=dummy_lib.entity.dummy_limb, tag=!dummy_lib.dummy_limb.torso, tag=!dummy_lib.dummy_limb.head, distance=..3] if function dummy_lib:utils/is_matching_guid run function dummy_lib:physics/wave_limb_strong
