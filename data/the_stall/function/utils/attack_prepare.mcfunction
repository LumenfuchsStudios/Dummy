## * AydenTFoxx @ 2025-01-15
## * 
## * Raises the dummy's arm to its signature attack.


# Remove walking tag
tag @s[tag=dummy_lib.dummy.is_walking] remove dummy_lib.dummy.is_walking

# Add score
execute unless score @s dummy_lib.clock matches 100.. run scoreboard players add @s dummy_lib.clock 2


# Add rotation
execute store result entity @s Rotation[1] float -1.0 run scoreboard players get @s dummy_lib.clock

# Set interpolation
data modify entity @s interpolation_duration set value 4


# Return 1 if attack is ready, 0 otherwise
execute unless score @s dummy_lib.clock matches 100.. run return 0

return 1